...
export const updateBookingFromCalendarEvent = internalAction({
  args: {
    eventId: v.string(),
  },
  handler: async (ctx, args) => {
    try {
      ...
      const bookings = await ctx.runQuery("getBookingByGoogleEventId", {
      ...
      if (Object.keys(updates).length > 0) {
        await ctx.runMutation("updateBooking", {
